...
import { addTeacher } from "@/app/dashboard/admin/users/actions";
...
export function AddTeacherModal() {
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        startTransition(async () => {
            const res = await addTeacher(null, formData);
            if (res.error) {
                setError(res.error);
            } else {
                setIsOpen(false);
    ...
    return (
        <>
            <Button 
                ...
                className="relative z-10 bg-emerald-600 hover:bg-emerald-700 text-white shadow-md font-semibold px-6 transition-all active:scale-95"
            >
                    ...
                    <div className="bg-white w-full max-w-lg shadow-2xl rounded-2xl flex flex-col relative animate-in zoom-in-95 duration-200 my-auto">
                        ...
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-emerald-50/50 rounded-t-2xl">
                            <div>
                                ...
                                <p className="text-xs text-gray-500 mt-1">Create a teacher profile and grant system access securely.</p>
                            </div>
                            <button 
                            ...
                            <div className="pt-2">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1 col-span-2">
                                        ...
                                        <Input name="qualification" required placeholder="M.A. Islamic Studies, B.Ed" />
                                    </div>
                                    <div className="space-y-1 col-span-2 sm:col-span-1">
                                        <label className="text-xs font-semibold text-gray-600">Experience (Years)</label>
                                        <Input name="experience" type="number" min="0" required placeholder="5" />
                                    </div>
                                    <div className="space-y-1 col-span-2 sm:col-span-1">
                                        <label className="text-xs font-semibold text-gray-600">Salary Monthly (₹)</label>
                                        <Input name="salary" type="number" min="0" step="0.01" required placeholder="25000" />
                                    </div>
                                </div>
                            </div>
